...
def main():
    ...
    logging.basicConfig(filename=data_params.log_dir + 'train-lc-{}.log'.format(model_training.model_id),
                        level=logging.INFO)
    ...
    pickle_save_record(train_record, file_name=data_params.save_prefix + '_train_record')
    pickle_save_record(valid_record, file_name=data_params.save_prefix + '_valid_record')
    pickle_save_record(ema_valid_record, file_name=data_params.save_prefix + '_ema_valid_record')
...
def train(train_label_loader, train_unlabel_loader, model, ema_model, optimizer, epoch, data_params):
    ...
    local_clustering_losses = AverageMeter()
    ...
    for i, ((unlabel_x, ema_unlabel_x), _) in enumerate(train_unlabel_loader):
        ...
        label_features, label_logits = model(label_x)
        unlabel_features, unlabel_logits = model(unlabel_x)
        ...
        # Compute local clustering loss
        if epoch >= data_params.lc_start_loss_epoch:
            local_clustering_loss = compute_local_clustering_cost(label_features, unlabel_features,
                                                                  data_params.lc_dist_threshold,
                                                                  dist_type='euclidean')

            loss += (local_clustering_loss * data_params.lc_weight *
                     sigmoid_rampup(epoch - data_params.lc_start_loss_epoch, data_params.lc_rampup_epochs))

            local_clustering_losses.update(local_clustering_loss.item())
    ...
    logging.info(('Training Epoch: {0}, '
              ...
              'Local clustering Loss {lc_losses.avg:.4f}, '
              ...
              .format(epoch, lr=optimizer.param_groups[-1]['lr'], top1=top1, top5=top5, losses=losses,
                      ...
                      ema_top5=ema_top5, lc_losses=local_clustering_losses)))

    return top1.avg


def compute_local_clustering_cost(label_features, unlabel_features, threshold, dist_type='euclidean'):
    if dist_type== 'euclidean':
        label_unlabel_loss = compute_pairwise_loss_in_euclidean(label_features, unlabel_features, threshold)
        unlabel_unlabel_loss = compute_pairwise_loss_in_euclidean(unlabel_features, unlabel_features, threshold)

        return label_unlabel_loss + unlabel_unlabel_loss
    else:
        NotImplementedError('This dist type {} has not been implemented.'.format(dist_type))


def compute_pairwise_loss_in_euclidean(f1, f2, threshold):
    batch_size1 = f1.size(0)
    batch_size2 = f2.size(0)
    pairwise_diff = f1.unsqueeze(1).expand(-1, batch_size2, -1) - f2.unsqueeze(0).expand(batch_size1, -1, -1)
    pairwise_l2 = torch.mean(pairwise_diff ** 2, dim=2)
    pairwise_l2_mask = torch.where(pairwise_l2 <= threshold,
                                   Variable(FloatTensor(batch_size1, batch_size2).fill_(1.0), requires_grad=False),
                                   Variable(FloatTensor(batch_size1, batch_size2).fill_(0.0), requires_grad=False))

    pairwise_l2_exp = torch.exp(-pairwise_l2 / float(threshold))
    return torch.sum(pairwise_l2 * pairwise_l2_mask * pairwise_l2_exp)/torch.sum(pairwise_l2_mask)
